    ...
    if diag_step == "input":
        ...
        with st.container(border=True):
            st.markdown(
                f"""
                <div style="
                    font-size:1.7rem; font-weight:800; color:#1e3a8a;
                    padding:0.6rem 0 0.7rem 0; margin-bottom:0.6rem;
...
def render_student_mode() -> None:
    ...
    with st.sidebar:
        ...
        # ── 🚪 로그아웃 (메뉴 바로 아래에 배치하여 학생 접근성 향상) ──
        if st.button("🚪 로그아웃", key="student_logout_btn_menu",
                     use_container_width=True,
                     help="현재 학생 세션을 종료하고 로그인 화면으로 돌아갑니다."):
...
with st.sidebar:
    ...
    if st.session_state.app_role == "student" and st.session_state.get("student_logged_in"):
        ...
        # ── 🔄 동기화 (한 줄, 강조 스타일 + 학생용 설명) ──
        st.markdown(
            '<div style="background:linear-gradient(180deg,#ecfeff 0%,#cffafe 100%);'
            'border:2px solid #0891b2;border-radius:10px;padding:0.55rem 0.8rem;'
            'margin:0.7rem 0 0.35rem 0;font-weight:800;color:#0e7490;font-size:1rem;'
            'box-shadow:0 1px 3px rgba(8,145,178,0.18);">'
            '🔄 동기화'
            '</div>',
            unsafe_allow_html=True,
        )
        if st.button("내 실습 기록 다시 불러오기", key="student_sync_btn", type="primary",
                      use_container_width=True,
                      help="구글 시트 history 탭에서 내 기록을 다시 끌어옵니다."):
            refreshed = refresh_my_history_cache()
            st.toast(f"누적 실습 이력 {len(refreshed)}건을 다시 불러왔어요.", icon="✅")
            st.rerun()
        st.caption("👉 내가 했던 실습 기록을 구글 시트에서 새로 받아오는 버튼이에요. 최신 기록이 안 보일 때 눌러주세요.")

        # ── 🔧 DB 저장 상태 (한 줄, 강조 스타일 + 학생용 설명) ──
        st.markdown(
            '<div style="background:linear-gradient(180deg,#eef2ff 0%,#e0e7ff 100%);'
            'border:2px solid #6366f1;border-radius:10px;padding:0.55rem 0.8rem;'
            'margin:0.9rem 0 0.35rem 0;font-weight:800;color:#3730a3;font-size:1rem;'
        ...
        render_db_diagnostic_panel()
        st.caption("👉 내 실습 결과가 구글 시트(DB)에 잘 저장됐는지 확인하는 버튼이에요. 저장이 안 됐을 때 눌러서 원인을 알 수 있어요.")
